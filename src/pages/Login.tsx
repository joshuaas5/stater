...
const Login: React.FC = () => {
  ...
  // Posições das partículas memorizadas para evitar re-render
  ...
  const handleForgotPassword = async (e: React.FormEvent) => {
    ...
    try {
      // Verificar se o email pertence a um usuario Google (nao pode recuperar senha)
      const { data: userData } = await supabase
        .from('users')
        .select('auth_provider')
        .eq('email', forgotData.email)
        .single();

      if (userData?.auth_provider === 'google') {
        showError('forgotEmail', 'Esta conta foi criada com Google. Use "Entrar com Google" para acessar.');
        setIsLoading(false);
        return;
      }
  ...
  return (
    <div className="edge-to-edge-login">      {/* Partículas flutuantes - posições memorizadas para evitar flicker */}
      ...
      <div className="absolute inset-0 no-flicker" />
