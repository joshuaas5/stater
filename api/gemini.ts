...
const handler = async (req: any, res: any) => {
  ...
  const fullPrompt = `Você é um consultor financeiro especialista em finanças pessoais e contabilidade. Use o contexto abaixo para responder à pergunta do usuário.\n\nContexto: ${financialContextText}\nPergunta: ${originalPrompt}\n\nSe a pergunta do usuário for para registrar, adicionar ou editar uma transação financeira (receita ou despesa), siga este formato:\n1. Primeiro, retorne APENAS um objeto JSON contendo os campos extraídos: tipo (receita ou despesa), descrição, valor, data (YYYY-MM-DD), categoria.\n2. Em seguida, escreva uma mensagem clara para o usuário confirmando a transação, por exemplo: \'Você confirma o registro da seguinte transação? ...\'\n\nPara qualquer outra pergunta, responda normalmente como um consultor financeiro, sem JSON.\n\nSe não houver saldo ou transações, apenas informe: 'Nenhuma transação encontrada para calcular saldo.'`;
