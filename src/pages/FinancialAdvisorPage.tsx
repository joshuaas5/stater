...
import { fixPortugueseAccents } from '@/utils/gemini';
...
const INITIAL_SYSTEM_MESSAGE_TEXT = `Ola! Sou o Stater IA - sua IA financeira completa!

O que posso fazer:
- Analisar extratos e documentos financeiros
- Registrar receitas/despesas automaticamente
- Ler e entender audios/mensagens de voz
- Fornecer dicas personalizadas de economia
- Controlar contas e planejamento financeiro

Como usar: Envie documentos, fotos, audios ou faca perguntas sobre suas financas. Sempre confirmo antes de salvar dados!
...
export const FinancialAdvisorPage: React.FC = () => {
  ...
  const handleAudioMessage = useCallback(async (audioBlob: Blob) => {
    ...
    // ?? TEMPORÁRIO: PAYWALL DE ÁUDIO REMOVIDO PARA TESTES
    // ?? TODO: Reativar paywall de áudio para produção
    /*
    ...
    if (!isPremium) {
      ...
      setMessages(prev => [...prev, {
        id: uuidv4(),
        text: `??? **Recurso PRO Necessário**\n\n? Áudios estão disponíveis apenas para usuários PRO.\n\n? **Assine o Stater PRO** por apenas R$ 14,90/mês:\n• ??? Envie áudios sempre que precisar\n• ?? Análise de PDFs e imagens\n• ?? Relatórios em diversos formatos\n• ?? Livre de anúncios\n• E muito mais!\n\n?? Faça upgrade para continuar!`,
        sender: 'assistant',
        timestamp: new Date(),
        avatarUrl: IA_AVATAR
      }]);
    ...
    */
    ...
    try {
      ...
      if (result.action === 'add_transaction' && result.amount && result.description) {
        console.log('🎤 [AUDIO_TRANSACTION] Transação detectada no áudio, abrindo modal');
        ...
        // CORRIGIDO: Usar openTransactionModal diretamente (forceCustomModalForAudio era um stub vazio!)
        ...
        setOriginalMessageForLearning(result.transcription || '');
        openTransactionModal(singleTransaction, {
          documentType: 'audio',
          establishment: 'Transação detectada por áudio'
        });
      ...
      // NOVO: Verificar se resultado tem transações em formato alternativo (hasTransaction + transactions)
      if ((result as any).hasTransaction && (result as any).transactions && (result as any).transactions.length > 0) {
        console.log('🎤 [AUDIO_TRANSACTION_ALT] Transações detectadas via formato alternativo:', (result as any).transactions);
        ...
        const transactionList = (result as any).transactions.map((tx: any) => ({
          type: tx.type === 'income' ? 'income' : 'expense',
          amount: parseFloat(tx.amount || tx.valor || 0),
          description: tx.description || tx.descrição || 'Transação via áudio',
          category: tx.category || tx.categoria || 'Outros',
        ...
        if (transactionList.length > 0) {
          ...
          const assistantMessage: ChatMessage = {
            id: uuidv4(),
            text: `✅ Detectei ${transactionList.length} transação(ões) no seu áudio! Confirme os detalhes no modal.`,
            sender: 'assistant',
            timestamp: new Date(),
            avatarUrl: IA_AVATAR
          };
  ...
  const handleSendMessage = async (message: string, skipAddingUserMessage = false) => {
    ...
    if (!safeMessage.trim() && waitingConfirmation && !pendingAction) {
      ...
      const errorMessage: ChatMessage = {
        id: uuidv4(),
        text: "?? Erro interno. Nenhuma ação pendente.",
        sender: 'system',
        timestamp: new Date()
      };
      ...
      try {        // Processar transações (OCR, texto, IA)
        if (pendingAction.tipo === 'generic_confirmation') {
          ...
          for (const transaction of validTransactions) {
            try {
              ...
              if (supabaseInsert.error) {
                console.warn('?? RPC falhou, usando insert tradicional:', supabaseInsert.error);
          ...
          if (actualSuccessCount > 0) {
            // Calcular total das transacoes QUE SERAO SALVAS (editaveis)
            const totalAmount = actualTransactionsToSave.reduce((sum, tx) => sum + tx.amount, 0);
            resultMessage += `${actualSuccessCount} transacao${actualSuccessCount > 1 ? 'es' : ''} processada${actualSuccessCount > 1 ? 's' : ''} com sucesso!\n`;
            resultMessage += `Total processado: ${formatBRL(totalAmount)}`;

            // Listar as transacoes processadas
            resultMessage += '\n\nTransacoes processadas:\n';
            actualTransactionsToSave.forEach((tx, index) => {
              const typeLabel = tx.type === 'income' ? 'Entrada' : 'Despesa';
              // Capitalizar primeira letra da descricao
              const capitalizedDescription = tx.description.charAt(0).toUpperCase() + tx.description.slice(1);
              resultMessage += `${index + 1}. ${typeLabel} de ${capitalizedDescription} - ${formatBRL(tx.amount)} (${tx.category})\n`;
            });
          } else {
            resultMessage += 'Nenhuma transacao valida foi processada.';
          }
        ...
        // Processar transações normais
          ...
          if (supabaseResult.error) {
            console.warn('?? RPC falhou, usando insert tradicional:', supabaseResult.error);
          ...
          setLoading(false);
          console.log('? [MODAL FIX] Estados resetados - modal deve fechar');
        ...
        if (waitingConfirmation || pendingAction) {
          console.log(' [MODAL FIX] Forçando reset de estados pendentes no finally...');
          setWaitingConfirmation(false);
          setPendingAction(null);
          setEditableTransactions([]);
        }
    ...
    // Se aguardando confirmação e usuário diz não ou cancelar
      ...
      // FIX: Também fechar modais visuais para evitar ações duplicadas
    ...
    try {
      ...
      // 🚀 EARLY DETECTION: Interceptar comandos simples de transação ANTES da chamada à IA
      // Isso garante que o modal abre imediatamente no celular sem esperar a resposta da IA
      const earlyTransactionMatch = message.match(/(?:adicione?|registre?|inclua|anote|coloque?|insira)\s+(?:r\$\s*)?(\d+(?:[,.]\d{1,2})?)\s*(?:reais?)?\s*(?:de\s+)?(.+)?/i);
      if (earlyTransactionMatch) {
        const rawAmount = earlyTransactionMatch[1].replace(',', '.');
        const amount = parseFloat(rawAmount);
        const descriptionHint = earlyTransactionMatch[2]?.trim() || '';

        if (amount > 0) {
          console.log('🚀 [EARLY_DETECT] Transação simples detectada ANTES da IA:', { amount, descriptionHint });

          // Determinar tipo baseado em palavras-chave
          let transactionType: 'income' | 'expense' = 'income'; // Default para entrada (adicione = adicionar dinheiro)
          let description = `Entrada de R$ ${amount.toFixed(2)}`;
          let category = 'Outras Receitas';

          const lowerHint = descriptionHint.toLowerCase();
          const expenseKeywords = ['gasto', 'despesa', 'saída', 'saida', 'mercado', 'supermercado', 'farmácia', 'farmacia',
            'combustível', 'combustivel', 'gasolina', 'posto', 'restaurante', 'comida', 'lanche', 'uber', 'taxi',
            'conta', 'boleto', 'aluguel', 'luz', 'água', 'agua', 'internet', 'celular', 'escola', 'faculdade',
            'academia', 'médico', 'medico', 'dentista', 'remédio', 'remedio', 'roupa', 'sapato', 'delivery', 'ifood'];
          const incomeKeywords = ['receita', 'entrada', 'salário', 'salario', 'freelance', 'venda', 'ganho', 'recebimento'];

          if (expenseKeywords.some(kw => lowerHint.includes(kw))) {
            transactionType = 'expense';
            description = descriptionHint || `Despesa de R$ ${amount.toFixed(2)}`;
            category = 'Não Categorizado';
          } else if (incomeKeywords.some(kw => lowerHint.includes(kw)) || !descriptionHint) {
            transactionType = 'income';
            description = descriptionHint || `Entrada de R$ ${amount.toFixed(2)}`;
            category = 'Outras Receitas';
          } else {
            // Se tem descrição mas não é claramente entrada/saída, assumir despesa (mais comum)
            transactionType = 'expense';
          ...
          console.log('🚀 [EARLY_DETECT] Abrindo modal diretamente:', singleTransaction);
          setOriginalMessageForLearning(message);
          openTransactionModal(singleTransaction, {
            documentType: 'ai_simple',
            establishment: 'Transação rápida'
          });

          // Adicionar mensagem do bot informando
          const botMsg: ChatMessage = {
            id: uuidv4(),
            text: `✅ Detectei sua transação! Preencha os detalhes no modal e confirme.`,
            sender: 'assistant',
            timestamp: new Date(),
            avatarUrl: IA_AVATAR
          };
...
FORMATO DE TEXTO OBRIGATÓRIO:
- SEMPRE use acentos e cedilha corretamente em português brasileiro
- Correto: transação, transações, descrição, não, ação, você, até
- INCORRETO: transacao, descricao, nao, acao, voce, ate
- Use R$ com formato brasileiro: R$ 1.234,56 (ponto para milhares, vírgula para centavos)
...
- Para consultas, responda em texto normal.`;
        ...
        if (typeof botResponseText === 'string') {
          // Corrigir acentos e cedilha que o modelo pode omitir
          botResponseText = fixPortugueseAccents(botResponseText);
        ...
        if (!isConsultaQuery) {
          try {
            ...
            if (looksLikeHtmlResponse) {
              ...
              botResponseText = "?? Recebi uma resposta inesperada da IA. Tente novamente em instantes.";
  ...
  const handleImageUpload = async (imageBase64: string) => {
    ...
    try {
      ...
      // ?? TODO: Reativar paywall para produção
      /*
      ...
      if (isPdf || !isTextFile) {
        ...
        if (!isPremium) {
          ...
          const mediaType = isPdf ? '?? PDFs' : '??? Imagens';
          setMessages(prev => [...prev, {
            id: uuidv4(),
            text: `${mediaType} **Recurso PRO Necessário**\n\n? Análise de ${isPdf ? 'PDFs' : 'imagens'} está disponível apenas para usuários PRO.\n\n? **Assine o Stater PRO** por apenas R$ 14,90/mês:\n• ${isPdf ? '?? PDFs sem preocupação com limite' : '??? Imagens sem preocupação com limite'}\n• ??? Envie áudios sempre que precisar\n• ?? Não se preocupe mais com limites de mensagens\n• ?? Relatórios em diversos formatos\n• ?? Livre de anúncios\n• E muito mais!\n\n?? Faça upgrade para continuar!`,
            sender: 'assistant',
            timestamp: new Date(),
            avatarUrl: IA_AVATAR
          }]);
      ...
      */
      ...
      const timeoutId = setTimeout(() => {
        if (abortControllerRef.current) {
          ...
          console.log(' Timeout de 40s atingido, cancelando requisição...');
        }
      ...
      if (isTextFile && fileData) {
        ...
        // Imagem/PDF tradicional - comprimir para ficar abaixo do limite do Vercel (4.5MB body)
        let processedBase64 = imageBase64.includes(',') ? imageBase64.split(',')[1] : imageBase64;
        const isPdfFile = imageBase64.startsWith('data:application/pdf');

        if (!isPdfFile) {
          // Comprimir imagem usando Canvas API
          try {
            const compressedBase64 = await new Promise<string>((resolve, reject) => {
              ...
              img.onload = () => {
                ...
                while (result.length > 3 * 1024 * 1024 && quality > 0.2) {
                  quality -= 0.1;
                  result = canvas.toDataURL('image/jpeg', quality).split(',')[1];
                }
                console.log(`[Compress] ${(processedBase64.length * 0.75 / 1024).toFixed(0)}KB → ${(result.length * 0.75 / 1024).toFixed(0)}KB`);
                resolve(result);
              };
              img.onerror = () => reject(new Error('Image load error'));
              img.src = imageBase64;
            });
            processedBase64 = compressedBase64;
          } catch (compressError) {
            console.warn('[OCR] Compressão falhou, usando original:', compressError);
          }
        }

        requestBody = {
          imageBase64: processedBase64,
          fileType: isPdfFile ? 'application/pdf' : 'image/jpeg',
          fileName: isPdfFile ? 'document.pdf' : 'photo.jpg',
        };
        console.log('[OCR] Enviando imagem/PDF para API, tamanho base64:', processedBase64.length);
      }

      console.log('[PRE_FETCH] ============================================');
      const maxUploadBytes = 6 * 1024 * 1024; // 6MB limite pra evitar OOM no WebView
      ...
      if (transactions.length === 0) {
        console.log(' Nenhuma transação encontrada no documento - permitindo adição manual');
        setMessages(prev => [...prev, {
          id: uuidv4(),
  ...
  const handleOFXUpload = async (file: File, content: string) => {
    ...
    try {
      ...
      const successMessage = `**Extrato OFX Processado!**\n\n` +
        ...
        `**Revise as transações** no modal e confirme para adicionar ao Stater!`;
    ...
    `}</style>
        ...
        <div
          ...
          style={{
            ...
            background: 'linear-gradient(180deg, #1e3a5f 0%, #0f172a 100%)', // Mobile background (desktop usa layout pai)
            minHeight: '100vh',
            width: '100vw',
            color: 'white',
            display: 'flex',
      ...
      `}</style>
          ...
          <div
            ...
            style={{
              ...
              alignItems: 'center',
              padding: 'calc(env(safe-area-inset-top, 20px) + 8px) 30px 16px 30px',
          ...
          <div
            ...
            style={{
              ...
              width: '100%',
              padding: '0 20px',
              paddingTop: 'calc(env(safe-area-inset-top, 0px) + 96px)', // Espaço para header fixo + safe area
              paddingBottom: '52px', // Espaço para ChatInput reduzido em 14px (66px ? 52px)
              boxSizing: 'border-box',
              minHeight: 'calc(100vh - 96px - env(safe-area-inset-top, 0px))' // Ajustado para header fixo
            }}
          >        {/* Chat Messages */}
            <ChatMessages
          ...
          {singleTransactionModal.isOpen && singleTransactionModal.transaction && (
            <div className="relative">
              <TransactionModal
                ...
                onClose={() => {
                  // FIX: Apenas resetar estados silenciosamente.
                  // NÃO mostrar "ação cancelada" aqui porque:
                  // - TransactionModal chama onClose ao fechar por QUALQUER motivo
                  // - Incluindo após onSave (sucesso), o que duplicava mensagens
                  // - O cancelamento explícito pelo usuário (texto "não") já tem seu próprio handler
                  console.log(' [MODAL_CLOSE] Resetando estados do modal');
                  setSingleTransactionModal({ isOpen: false, transaction: null });
                  setEditableTransactions([]);
                  setCurrentTransactionIndex(0);
                  setWaitingConfirmation(false);
                ...
                onSave={async (transactionData) => {
                  ...
                  if (currentTransactionIndex < editableTransactions.length - 1) {
                    ...
                    // FIX: Fechar modal ANTES e resetar waitingConfirmation
                    // para que handleSendMessage('sim') processe o salvamento correto
                    setSingleTransactionModal({ isOpen: false, transaction: null });
                    setCurrentTransactionIndex(0);
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <div className="flex gap-1">
                      {editableTransactions.map((_, idx) => (
                        <div
                          key={idx}
                          className={`w-2 h-2 rounded-full transition-all ${idx === currentTransactionIndex
                            ? 'bg-blue-600 w-6'
          ...
          <MultiTransactionModal
            ...
            onClose={() => {
              ...
              setMultiTransactionModal({ isOpen: false, transactions: [], documentInfo: {} });
              // FIX: Também resetar estados de confirmação para evitar conflitos
              setWaitingConfirmation(false);
              setPendingAction(null);
              setEditableTransactions([]);
              setMessages(prev => [
            ...
            onSaveAll={async (transactions) => {
              ...
              try {
                ...
                const errorMessage = errorCount > 0
                  ? `${errorCount} ${errorCount === 1 ? 'transação falhou' : 'transações falharam'}.`
                  : '';
...
function getProcessingIcon(content: string): string {
  ...
  if (content.includes('imagem')) return '[IMG]';
  if (content.includes('texto')) return '[TXT]';
  return '[DOC]';
}
