...
export const showBillNotifications = (): void => {
  ...
  const importantNotifications = notifications.filter(notification => {
    ...
    // APENAS notificações do dia - SEM atraso, SEM semana
    if (notification.type === 'week_warning' || notification.type === 'overdue') {
      return false; // Remover notificações de 7 dias e atraso
    }
