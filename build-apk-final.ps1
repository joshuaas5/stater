...
Write-Host "Step 4: Building APK Android..." -ForegroundColor Yellow
...
if (Test-Path $APK_SOURCE) {
    ...
    Write-Host "APK nao encontrado em: $APK_SOURCE" -ForegroundColor Red
    Write-Host "Listando arquivos de output:" -ForegroundColor Yellow
    Get-ChildItem "android\app\build\outputs" -Recurse -Name "*.apk" -ErrorAction SilentlyContinue | ForEach-Object { Write-Host "   - $_" }
    exit 1
}

# Step 6: Relatorio final
Write-Host ""
...
Write-Host "Assinado: SIM" -ForegroundColor Green
Write-Host ""
Write-Host "APK pronto para instalacao!" -ForegroundColor Cyan
