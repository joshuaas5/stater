-- Execute este SQL no Supabase para criar as tabelas do Telegram
...
CREATE POLICY "Usuários podem deletar apenas seus próprios códigos" ON telegram_link_codes
...
CREATE POLICY "Usuários podem deletar apenas suas próprias vinculações" ON telegram_users
  FOR DELETE USING (auth.uid() = user_id);
...
-- Limpeza automática de códigos expirados (opcional - executar periodicamente)
