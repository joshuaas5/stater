...
async function handleAudio(msg) {
    ...
    try {
        ...
        console.log('🎤 [AUDIO] URL do áudio obtida:', fileUrl);

        const audioResponse = await axios.get(fileUrl, { responseType: 'arraybuffer', timeout: 30000 });
        const audioBuffer = Buffer.from(audioResponse.data);

        // Check audio size - Vercel has payload limits
        const audioSizeKB = audioBuffer.length / 1024;
        console.log(`🎤 [AUDIO] Áudio baixado: ${audioSizeKB.toFixed(1)} KB`);

        // If audio is too large (>4MB), skip inline processing
        if (audioBuffer.length > 4 * 1024 * 1024) {
            await deleteMessage(chatId, processingMsg.data.result.message_id);
            await sendMessage(chatId, '🎤 *Áudio muito grande!*\n\nPor favor, envie um áudio mais curto (até 1 minuto) ou digite sua transação.\n\nExemplo: "gastei 50 no mercado"', { parse_mode: 'Markdown' });
            return;
        }

        const audioBase64 = audioBuffer.toString('base64');
        // Clean mime type - Telegram sends "audio/ogg; codecs=opus" but Gemini wants clean type
        let mimeType = audioFile.mime_type || 'audio/ogg';
        if (mimeType.includes(';')) {
            mimeType = mimeType.split(';')[0].trim();
        }
        console.log(`🎤 [AUDIO] Tipo original: ${audioFile.mime_type}, limpo: ${mimeType}`);

        // Send audio to Gemini for transcription and analysis
        const prompt = `Voce e um assistente financeiro brasileiro. Analise este audio e:

1. TRANSCREVA o que a pessoa disse em portugues brasileiro
2. Se mencionar TRANSACOES FINANCEIRAS (valores, gastos, receitas), extraia-as
3. Se for uma PERGUNTA sobre financas, responda de forma util
4. Se nao houver conteudo financeiro, responda naturalmente

IMPORTANTE: Use acentos e cedilha corretos (transacao, descricao, nao, voce)

Se detectar transacoes, retorne APENAS JSON no formato:
{"transcription": "texto transcrito", "hasTransaction": true, "transactions": [{"description": "descricao", "amount": 50.0, "type": "expense", "category": "Outros", "date": "${new Date().toISOString().split('T')[0]}"}]}

Se NAO detectar transacoes, retorne:
{"transcription": "texto transcrito", "hasTransaction": false, "response": "sua resposta ao usuario"}

IMPORTANTE: Responda APENAS com JSON valido, sem texto adicional.`;

        console.log('🎤 [AUDIO] Enviando para Gemini 2.0 Flash...');
        let geminiResponse;
        try {
            ...
            console.error('❌ [AUDIO] Erro na chamada Gemini:', geminiError.response?.status, geminiError.response?.data?.error?.message || geminiError.message);

            // Fallback: Try with gemini-1.5-flash which has better audio support
            console.log('🎤 [AUDIO] Tentando fallback com gemini-1.5-flash...');
            try {
                geminiResponse = await axios.post(
                ...
                console.log('🎤 [AUDIO] Fallback com gemini-1.5-flash funcionou!');
            } catch (fallbackError) {
                console.error('❌ [AUDIO] Fallback também falhou:', fallbackError.response?.status, fallbackError.response?.data?.error?.message || fallbackError.message);
                throw new Error(`Gemini API rejeitou o áudio: ${geminiError.response?.data?.error?.message || geminiError.message}`);
            }
        }
        ...
        console.log('🎤 [AUDIO] Resposta Gemini:', responseText);
        ...
        if (jsonMatch) {
            try {
                ...
                if (parsed.hasTransaction && parsed.transactions && parsed.transactions.length > 0) {
                    // Save pending transactions
                    await supabase
                        .from('telegram_users')
                        .update({ pending_transactions: parsed.transactions })
                        .eq('telegram_chat_id', chatId.toString());

                    let txMessage = `🎤 *Áudio processado com sucesso!*\n\n`;
                    if (parsed.transcription) {
                        txMessage += `📝 *Transcrição:* _"${parsed.transcription}"_\n\n`;
                    }
                    txMessage += `💰 *Transações detectadas:*\n\n`;

                    parsed.transactions.forEach((tx, i) => {
                        const emoji = tx.type === 'income' ? '📈' : '📉';
                        txMessage += `${emoji} ${tx.description} - R$ ${Number(tx.amount).toFixed(2)}\n`;
                    });

                    txMessage += `\n❓ *Deseja salvar essas transações?*\nResponda *SIM* ou *NÃO*`;

                    await sendMessage(chatId, txMessage, {
                        parse_mode: 'Markdown',
                        reply_markup: {
                            keyboard: [[
                                { text: '✅ Confirmar' },
                    ...
                    // No transaction, just a response
                    const responseMessage = parsed.response || parsed.transcription || 'Não consegui entender o áudio. Tente novamente.';
                    let audioReply = `🎤 *Áudio processado!*\n\n`;
                    if (parsed.transcription) {
                        audioReply += `📝 _"${parsed.transcription}"_\n\n`;
                    }
                    audioReply += `🤖 ${responseMessage}`;
                    await sendMessage(chatId, audioReply, { parse_mode: 'Markdown' });
                }
            } catch (parseError) {
                console.error('❌ [AUDIO] Erro ao parsear JSON:', parseError);
                // Fallback: send the raw text
                const cleanResponse = responseText.replace(/```json/g, '').replace(/```/g, '').trim();
                await sendMessage(chatId, `🎤 *Áudio processado!*\n\n${cleanResponse}`, { parse_mode: 'Markdown' });
            }
        } else {
            // No JSON in response, send as text
            const cleanResponse = responseText.replace(/\*/g, '').replace(/#/g, '').trim();
            await sendMessage(chatId, `🎤 *Áudio processado!*\n\n${cleanResponse}`, { parse_mode: 'Markdown' });
        }

    } catch (error) {
        try { await deleteMessage(chatId, processingMsg.data.result.message_id); } catch (e) { }
        console.error('❌ [AUDIO] Erro ao processar áudio:', error.response?.data || error.message);
        await sendMessage(chatId, '😥 *Erro ao processar áudio.*\n\n💡 Tente enviar novamente ou digite sua transação manualmente.\n\nExemplo: "gastei 50 no mercado"', { parse_mode: 'Markdown' });
    }
}
...
async function handleTextMessage(msg) {
    ...
    if (userSession) {
        ...
        // Premium check REMOVIDO - Telegram bot livre para todos os usuarios
...
- NUNCA mencione planos premium, assinaturas ou preços
- NUNCA diga que algum recurso não está disponível
- Todos os recursos estão HABILITADOS para o usuário
