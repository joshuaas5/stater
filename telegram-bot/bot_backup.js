...
Retorne APENAS o array JSON, sem explicações adicionais.`;
        ...
        if (jsonMatch) {
            ...
            transactions.forEach(t => {
                ...
                if (t.categoria) {
                    ...
                    if (!categoriasValidas.includes(categoriaLower)) {
                        console.log(`⚠️ Categoria "${t.categoria}" não reconhecida, usando "outros"`);
                        t.categoria = 'outros';
                    } else {
            ...
            console.log(`✅ ${transactions.length} transações extraídas e validadas`);
            return { transactions };
        }
...
async function confirmTransactions(chatId) {
    ...
    try {
        ...
        if (userId && salvasComSucesso > 0) {
            // Buscar saldo ATUALIZADO após salvar as transações
...
async function processChatMessage(chatId, message, userSession) {
    try {
        ...
        if (isTransactionRequest) {
            ...
            // Mostrar mensagem bonita de detecção (SEM "Ouvi" para texto)
            const detectionMsg = await bot.sendMessage(chatId, `🤔 *Processando transação com IA...*`, { parse_mode: 'Markdown' });
